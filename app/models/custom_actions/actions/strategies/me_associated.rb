...
#   - "user_custom_field_<id>" -> the value of a user-format custom field
#
...
module CustomActions::Actions::Strategies::MeAssociated
  ...
  def transformed_value_with_wp(val, work_package)
    ...
    when RESPONSIBLE_VALUE_KEY
      ...
      custom_field_id = user_custom_field_value_id(val)
      custom_field_id ? resolve_user_custom_field(work_package, custom_field_id) : val
    end
  end
  ...
  def special_value_marker?(value)
    value = value.to_s
    built_in_value_markers.include?(value) || user_custom_field_value_id(value).present?
  end
